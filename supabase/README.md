...
| `order-status` | лендінг (полінг після відкриття оплати) | read-only: `GET ?token=` → `{ok, status, botLink}`; погасити токен не може; лендінг показує «дякуємо за покупку», коли статус стає `paid`/`claimed` |
...
supabase db push
supabase functions deploy create-order wayforpay-callback claim payment-return order-status
```
